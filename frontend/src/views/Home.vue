<template>
  <div class="home">
    <div class="welcome-header">
      ...
      <div class="stats-bar">
        <span class="stat-item"><span class="stat-dot green"></span> 话题 {{ stats.topicCount || 0 }}</span>
        <span class="stat-sep">/</span>
        <span class="stat-item"><span class="stat-dot blue"></span> 帖子 {{ stats.postCount || 0 }}</span>
        <span class="stat-sep">/</span>
        <span class="stat-item"><span class="stat-dot purple"></span> 用户 {{ stats.userCount || 0 }}</span>
        <span class="stat-sep">/</span>
        <span class="stat-item"><span class="stat-dot orange"></span> 互动 {{ stats.interactionCount || 0 }}</span>
      </div>
    </div>
    ...
    <div class="day-marker" v-if="activeTab === 'latest'"><span class="marker-icon">✨</span> 今天</div>
    <div class="topic-list">
      ...
      <div v-for="item in filteredArticles" :key="item.id" class="topic-row" @click="$router.push('/article/' + item.id)">
        <div class="topic-cover" v-if="item.coverImage">
          <img :src="item.coverImage" :alt="item.title" @error="handleImgError" />
        </div>
        <div class="topic-cover placeholder" v-else>
          <span>{{ getCategoryEmoji(item.categoryName) }}</span>
        </div>
        ...
        <div class="topic-author">
          <div class="author-avatar" v-if="item.authorAvatar"><img :src="item.authorAvatar" @error="handleAvatarError" /></div>
          <div class="author-avatar" :class="'avatar-' + getTagColor(item.authorName)" v-else>{{ (item.authorName || '匿').charAt(0) }}</div>
...
export default {
  ...
  data() {
    return {
      articles: [], total: 0, currentPage: 1, pageSize: 10, activeTab: 'latest',
  ...
  async created() { await this.loadArticles(); this.stats.topicCount = this.total || 0 },
  methods: {
    async loadArticles() {
      const res = await articleApi.getList({ page: this.currentPage, size: this.pageSize })
      this.articles = (res.data.records || []).map(a => ({ ...a, coverImage: a.coverImage || a.cover_image || '', authorAvatar: a.authorAvatar || a.userAvatar || '' }))
    ...
    formatTime(time) {
      ...
      return Math.floor(diff / 60000) > 0 ? Math.floor(diff / 60000) + '分钟前' : '刚刚'
    },
    getTagColor(name) {
    ...
    getCategoryEmoji(cat) {
      const map = { '养猫指南': '🐱', '养狗指南': '🐕', '宠物健康': '🏥', '宠物饮食': '🍽️', '宠物趣事': '😄', '异宠乐园': '🐰', '水族观赏': '🐠', '宠物摄影': '📸', '救助领养': '❤️', '宠物用品': '🛒' }
      return map[cat] || '🐾'
    },
    handleImgError(e) { e.target.style.display = 'none'; e.target.parentElement.style.background = 'linear-gradient(135deg, #e7f3ff, #f0f0ff)' },
...
<style scoped>
.home { padding: 0; }
.welcome-header { text-align: center; padding: 40px 20px 24px; background: var(--surface); border-radius: var(--rounded-lg); box-shadow: var(--shadow); margin-bottom: 20px; }
.forum-title { font-size: 36px; font-weight: 800; color: var(--ink); margin-bottom: 8px; }
.forum-subtitle { font-size: 16px; color: var(--ink-secondary); margin-bottom: 20px; }
.stats-bar { display: flex; align-items: center; justify-content: center; gap: 6px; font-size: 14px; color: var(--ink-secondary); }
...
.content-tabs { display: flex; justify-content: center; background: var(--surface); border-radius: var(--rounded-lg); box-shadow: var(--shadow); margin-bottom: 20px; padding: 4px; }
.tab-btn { padding: 10px 28px; border: none; background: none; color: var(--ink-secondary); font-size: 15px; font-weight: 500; cursor: pointer; border-radius: var(--rounded-md); transition: all 0.15s; }
.tab-btn:hover { background: var(--canvas); color: var(--ink); }
.tab-btn.active { background: var(--primary); color: #fff; font-weight: 600; }
.day-marker { display: flex; align-items: center; gap: 6px; padding: 14px 0; font-size: 14px; font-weight: 600; color: var(--ink); border-bottom: 1px solid var(--border); margin-bottom: 0; }
.marker-icon { font-size: 16px; }
.topic-header { display: flex; align-items: center; padding: 12px 0; border-bottom: 1px solid var(--border); font-size: 13px; font-weight: 600; color: var(--ink-secondary); }
.col-title { flex: 1; }
...
.topic-row { display: flex; align-items: flex-start; padding: 16px 0; border-bottom: 1px solid var(--border-light); cursor: pointer; transition: background 0.1s; gap: 12px; }
.topic-row:hover { background: var(--primary-soft); border-radius: var(--rounded-md); }
.topic-cover { width: 80px; height: 60px; min-width: 80px; border-radius: var(--rounded-md); overflow: hidden; background: var(--canvas); display: flex; align-items: center; justify-content: center; }
.topic-cover img { width: 100%; height: 100%; object-fit: cover; }
.topic-cover.placeholder { font-size: 28px; background: linear-gradient(135deg, var(--primary-soft), #f0f0ff); }
.topic-main { flex: 1; min-width: 0; }
.topic-title { font-size: 16px; font-weight: 600; color: var(--ink); margin-bottom: 6px; line-height: 1.4; }
.topic-tags { display: flex; gap: 6px; margin-bottom: 6px; flex-wrap: wrap; }
.tag { display: inline-block; padding: 2px 10px; border-radius: var(--rounded-full); font-size: 12px; font-weight: 500; }
...
.author-name { font-size: 12px; color: var(--ink-secondary); text-align: center; }
.topic-replies, .topic-views, .topic-activity { width: 70px; text-align: center; font-size: 14px; color: var(--ink-secondary); padding-top: 6px; }
