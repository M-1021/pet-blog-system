  ...
  <div class="publish">
    ...
    <div class="publish-card">
      <el-form :model="form" :rules="rules" ref="pubForm" label-width="80px">
        ...
        <el-form-item label="内容" prop="content"><el-input v-model="form.content" type="textarea" :rows="12" placeholder="请输入文章内容"></el-input></el-form-item>
        <el-form-item>
...
export default {
  name: 'Publish',
  data() { return { form: { title: '', content: '', categoryId: '' }, categories: [], isEdit: false, rules: { title: [{ required: true, message: '请输入标题', trigger: 'blur' }], categoryId: [{ required: true, message: '请选择分类', trigger: 'change' }], content: [{ required: true, message: '请输入内容', trigger: 'blur' }] } } },
  async created() { const catRes = await categoryApi.getList(); this.categories = catRes.data; const editId = this.$route.query.id; if (editId) { this.isEdit = true; const res = await articleApi.getDetail(editId); this.form = { id: res.data.id, title: res.data.title, content: res.data.content, categoryId: res.data.categoryId } } },
  methods: { async handleSubmit() { try { await this.$refs.pubForm.validate(); if (this.isEdit) { await articleApi.update(this.form); this.$message.success('修改成功') } else { await articleApi.create(this.form); this.$message.success('发布成功') }; this.$router.push('/my-articles') } catch (e) {} } }
}
</script>
<style scoped>
